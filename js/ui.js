...
const UI = (() => {
      ...
      entries.forEach((entry, i) => {
        ...
        item.innerHTML = `
          <div class="relative w-full aspect-[3/2] rounded-md overflow-hidden bg-gray-200 dark:bg-gray-700 ${isBefore ? 'ring-2 ring-yellow-500' : ''}">
            ...
            ${getCandidateBadge(entry, 'mobile')}
          </div>
  ...
  // --- ユーティリティ ---
  // 候補数バッジHTML生成（モバイル/デスクトップ共通）
  function getCandidateBadge(entry, size = 'desktop') {
    const count = entry.candidates?.results?.length ?? 0;
    if (count === 0) return '';
    const cls = size === 'mobile'
      ? 'w-4 h-4 text-[8px]'
      : 'z-10 w-5 h-5 text-[9px]';
    return `<span class="absolute bottom-0.5 right-0.5 ${cls} flex items-center justify-center font-bold bg-purple-500 text-white rounded-full leading-none">${count}</span>`;
  }
